...
function tryReadCover(zip, entry) {
  try {
    ...
    const mimeSubtype = normalizeMimeSubtype(dimensions.type);

    return {
      type: mimeSubtype,
      size: imgBuffer.length,
      value: `data:image/${mimeSubtype};base64,${imgBuffer.toString('base64')}`
    };
  } catch (error) {
    return null;
  }
